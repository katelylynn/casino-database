...
select schema_name(tab.schema_id) as schema_name,
       ...
       case when ep.value is not null then ep.value
       ...
       case when t.is_user_defined = 0 then 
		...
		case when fk.parent_table is not null then fk.parent_table
			 else '' end as referenced_table,
   ...
   from sys.tables as tab
        ...
        left join (
                  select fc.parent_column_id, 
                         fc.parent_object_id,
						 tab_prim.name AS parent_table
                    from sys.foreign_keys as f 
                         inner join sys.foreign_key_columns as fc 
                             on f.object_id = fc.constraint_object_id
					     inner join sys.tables as tab_prim
							 on f.referenced_object_id = tab_prim.object_id
                   group by fc.parent_column_id, fc.parent_object_id, tab_prim.name
                  ) as fk
